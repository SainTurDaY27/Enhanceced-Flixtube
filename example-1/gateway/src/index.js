...
function setupHandlers(app) {
    ...
    // HTTP GET API to advertising to the user's browser.
    app.get("/advertising", (req, res) => {
        http.request( // Gets the advertising from the advertising microservice.
            {
                host: `advertising`,
                path: `/advertising`,
                method: `GET`,
            ...
            (response) => {
                ...
                response.on("end", () => {
                    // res.send(JSON.parse(data).advertising);
                    res.render("advertising", { advertising: JSON.parse(data).advertising });
                });

                response.on("error", err => {
                    console.error("Failed to get advertising.");
                    console.error(err || `Status code: ${response.statusCode}`);
                    res.sendStatus(500);
                });
            }
